...
export default function TransactionsPage() {
  ...
  const FilterGroup = ({ label, opts, value, set }: { label: string; opts: string[]; value: string; set: (v: string) => void }) => (
    <div className="space-y-1">
      <p className="text-white/20 text-[9px] font-black uppercase tracking-widest">{label}</p>
      <div className="flex gap-2 flex-wrap">
        {opts.map(o => (
          <button key={o} onClick={() => set(o)}
            className={`px-3.5 h-8 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${value === o ? "bg-[#00FF85] text-black" : "bg-white/[0.04] border border-white/[0.08] text-white/40 hover:border-white/20"}`}>
            {o}
          </button>
        ))}
      </div>
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: "Total Sent",     value: `$${totalSent.toLocaleString("en-US", { minimumFractionDigits: 2 })}`,     color: "text-red-400",    sub: `${txns.filter(t => t.direction === "SENT").length} transfer(s)` },
          { label: "Total Received", value: `$${totalReceived.toLocaleString("en-US", { minimumFractionDigits: 2 })}`, color: "text-[#00FF85]",  sub: `${txns.filter(t => t.direction === "RECEIVED").length} deposit(s)` },
          { label: "Pending",        value: String(pendingCount),                                                       color: "text-yellow-400", sub: "Awaiting processing" },
        ].map(s => (
      ...
      <div className="rounded-2xl bg-white/[0.03] border border-white/[0.06] overflow-hidden">
        {loading ? (
          <div className="py-20 flex justify-center"><div className="h-8 w-8 border-2 border-white/10 border-t-[#00FF85] rounded-full animate-spin" /></div>
        ) : filtered.length === 0 ? (
            ...
            {filtered.map((tx: any) => (
              <div key={tx.id} className="flex items-center justify-between p-4 hover:bg-white/[0.02] transition-colors">
                <div className="flex items-center gap-3">
                  <div className={`h-10 w-10 rounded-full flex items-center justify-center font-black text-base shrink-0 ${tx.direction === "RECEIVED" ? "bg-[#00FF85]/10 text-[#00FF85]" : "bg-red-500/10 text-red-400"}`}>
                  ...
                  <div>
                    ...
                    <p className="text-white/20 text-[10px] mt-0.5 font-mono">{tx.direction === "SENT" ? `To: ${tx.recipientAccount}` : "From: Fortress Transfer"}</p>
                  </div>
                </div>
                <div className="text-right shrink-0">
                  ...
                  <span className={`inline-block mt-1 px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest ${tx.status === "COMPLETED" ? "bg-[#00FF85]/10 text-[#00FF85]" : tx.status === "PENDING" ? "bg-yellow-500/10 text-yellow-400" : "bg-red-500/10 text-red-400"}`}>
                    {tx.status}
                  </span>
                </div>
              </div>
